...
def get_subject(subject):
    ...
    current = conn.execute(
        '''
        SELECT b.POSITION, b.FULL_NAME, b.GENDER, b.WEALTH, c.NAME, b.SOURCE, i.INDUSTRY, b.ID
        ...
        ;
        ''', (subject, )).fetchone()
    next_and_prev = conn.execute(
        '''
        SELECT FULL_NAME
        FROM BILLIONAIRES 
        WHERE ID IN (?, ?) 
        ;
        ''', (current[7] - 1, current[7] + 1)).fetchall()
    conn.commit()
    conn.close()
    return render_template('subject/subject.html', current=current, previous=next_and_prev[0], next=next_and_prev[1])
